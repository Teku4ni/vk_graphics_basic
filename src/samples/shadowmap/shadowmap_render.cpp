...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  {
    .extent = vk::Extent3D{m_width * m_buffer_scale, m_height * m_buffer_scale, 1},
  ...
  {
    .extent = vk::Extent3D{2048 * m_buffer_scale, 2048 * m_buffer_scale, 1},
    .name = "shadow_map",
    .format = vk::Format::eD16Unorm,
    .imageUsage = vk::ImageUsageFlagBits::eDepthStencilAttachment | vk::ImageUsageFlagBits::eSampled
  });

  superSampledImage = m_context->createImage(etna::Image::CreateInfo
  {
    .extent = vk::Extent3D{m_width * m_buffer_scale, m_height * m_buffer_scale, 1},
    .name = "superSampledImage",
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_pBindings->BindImage(0, superSampledImage.getView({}), linearSampler.get(), VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
...
{
  if (m_buffer_scale != (m_SSAA ? 2 : 1))
  {
    m_buffer_scale = m_SSAA ? 2 : 1;
    mainViewDepth.reset();
    shadowMap.reset();
    superSampledImage.reset();
  ...
  //
  {
    etna::RenderTargetState renderTargets(a_cmdBuff, {2048 * m_buffer_scale, 2048 * m_buffer_scale}, {}, shadowMap);
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, {m_width * m_buffer_scale, m_height * m_buffer_scale}, {{superSampledImage}}, mainViewDepth);
  ...
  etna::set_state(a_cmdBuff, superSampledImage.get(), vk::PipelineStageFlagBits2::eFragmentShader,
    vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal,
    vk::ImageAspectFlagBits::eColor);

  etna::flush_barriers(a_cmdBuff);
  {
    auto outputInfo = etna::get_shader_program("simple_SSAA");
    auto set = etna::create_descriptor_set(outputInfo.getDescriptorLayoutId(0), a_cmdBuff,
    {
      etna::Binding{0, superSampledImage.genBinding(linearSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal)},
    });
    VkDescriptorSet vkSet = set.getVkSet();
