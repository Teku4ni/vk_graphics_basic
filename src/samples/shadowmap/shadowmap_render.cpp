...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("simple_gbuffers",
    {VK_GRAPHICS_BASIC_ROOT"/resources/shaders/gbuffers.frag.spv", VK_GRAPHICS_BASIC_ROOT"/resources/shaders/simple.vert.spv"});
  etna::create_program("simple_deferred_shading",
    { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/new_quad.vert.spv", VK_GRAPHICS_BASIC_ROOT "/resources/shaders/deferred_shading.frag.spv" });
}
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_gbuffersPipeline = pipelineManager.createGraphicsPipeline("simple_gbuffers",
    {
      .vertexShaderInput = sceneVertexInputDesc,
  ...
  m_deferredPipeline = pipelineManager.createGraphicsPipeline("simple_deferred_shading",
    { 
      .depthConfig = 
      {
...
void SimpleShadowmapRender::DrawSceneCmd(VkCommandBuffer a_cmdBuff, const float4x4& a_wvp)
{
  ...
  for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); ++i)
  {
    ...
    vkCmdPushConstants(a_cmdBuff, m_gbuffersPipeline.getVkPipelineLayout(),
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  {
    auto deferredShadingInfo = etna::get_shader_program("simple_gbuffers");

    auto set = etna::create_descriptor_set(deferredShadingInfo.getDescriptorLayoutId(0), {
      etna::Binding{ 0, vk::DescriptorBufferInfo{ constants.get(), 0, VK_WHOLE_SIZE } },
    });

    VkDescriptorSet vkSet = set.getVkSet();

    etna::RenderTargetState renderTargets(a_cmdBuff, { m_width, m_height }, { { gBuffer.albedo.getView({}), gBuffer.normals.getView({}) } }, mainViewDepth.getView({}));

    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_gbuffersPipeline.getVkPipeline());
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_gbuffersPipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);
  ...
  //
  {
    auto deferredShadingInfo = etna::get_shader_program("simple_deferred_shading");
    ...
    vkCmdPushConstants(a_cmdBuff, m_deferredPipeline.getVkPipelineLayout(), VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pushConstDeferred), &pushConstDeferred);
