...
void main()
{
  ...
  if (Params.VSMFlag) 
  {
    ...
    float mean = vsm.x;
    float deviation = posLightSpaceNDC.z - mean;
    float variance = vsm.y - mean * mean;

    float val = variance / (variance + deviation * deviation);
    shadow = ((posLightSpaceNDC.z < mean + 0.001f) || outOfView) ? 1.0f : val;
  }
  else
