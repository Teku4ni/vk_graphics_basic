...
void main()
{
  ...
  const float lightAngleCos = dot(normalize(Params.spotDir), -lightDir);
  const float intensity = smoothstep(cos(radians(Params.lightOuterCutOff)), cos(radians(Params.lightInnerCutOff)), lightAngleCos);
